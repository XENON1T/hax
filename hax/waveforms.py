...
def inspect_events(events, focus='all'):
    ...
    for event in events:
        ...
        xed_filename = 'xe100_%06d_%04d_%06d.xed' % (1e4 * int(event.dataset_number / 1e4),
                                                     event.dataset_number % 1e4,
                                                     int(event.event_number / 1e6))
...
def _inspect_event_xenon100(event_number, xed_filename, focus='all'):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as f:
        ...
        if CONFIG['raw_data_access_mode'] == 'local':
            filename = CONFIG['raw_data_location'].format(dataset_name=dataset_name, xed_filename=xed_filename)
