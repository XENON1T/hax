...
class DataExtractor():
    ...
    def __init__(self):
        ...
        self.data = []
        # Default values for cuts: everything passes
        ...
        if eval(self.event_cut_string):
            ...
            for peak in event.peaks:
                ...
                if eval(self.peak_cut_string):
                    ...
                    #peak_entry = np.array([getattr(peak,field) for field in self.peak_fields])
                    ...
                    if self.level == 'hit':                        
                        ...
                        if hit_entry is None :
                            ...
                            raise ValueError("Unable to read hit info. Is it in the root file? (Try: peak_cuts=['type == 's1''])")
    ...
    def get_data(self, dataset, level ='peak', event_fields = ['event_number'], peak_fields=['area','hit_time_std'], hit_fields = [],
        ...
        #self.peak_fields = [field.replace('range_50p_area','range_area_decile[5]') for field in peak_fields]
        ...
        if level == 'hit':
            ...
            field_names = event_fields + ['peak_' + field for field in peak_fields] + ['hit_' + field for field in hit_fields]
        if level == 'peak':
